 ...
 * path the parent passed via {@link OUTPUT_PATH_ENV}. The agent never sees the
 * path; it just calls the tool once with its findings.
 */
...
import { OUTPUT_PATH_ENV, SUBMIT_RESULT_TOOL } from "./constants.ts";
...
export const submitResultTool = defineTool({
	...
	async execute(_toolCallId, params) {
		...
		if (!outputPath) {
			return text(`Cannot submit result: ${OUTPUT_PATH_ENV} is not set in this process.`);
		}
		try {
