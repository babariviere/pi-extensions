...
Attempts run in a fresh Herdr pane through a transient `systemd-run --user --pty --wait --collect` service. The service uses an empty root namespace, an allowlisted environment, explicit read-only runtime/input binds, and attempt-owned writable paths. The primary checkout and its Git metadata are never writable by an agent. Workers commit inside an isolated per-attempt clone, then the controller validates and imports only the exact candidate commit under the repository lock. Evidence replay runs credential-free with a sterile home and no network. The controller, not an agent, performs GitHub mutations and authenticated CI verification.
...
The controller creates deterministic branches named `background/<case-id>/<ordinal>`, imports an exact candidate commit from one isolated attempt clone per PR, pushes through controller-owned effects, creates draft PRs, edits PR metadata, links stack branches with `gh stack link <bottom-branch> <next-branch> ...`, and runs `gh pr ready <number>` only after verification and required CI pass. Before readiness it locks the GitHub head branch through the branch-lock API, rechecks the exact head and base, and retains the lock after success. A human reviewer must explicitly unlock the branch in GitHub before further pushes. The controller requires permission to lock and unlock repository branches and fails closed if that permission is absent. There is no merge command. Stack items execute bottom to top; do not run concurrent writers on one linear stack.
...
Agent services use deny-by-default filesystem and environment isolation, but this remains a defense-in-depth boundary that must be validated on the deployed Linux/systemd version. Workers use isolated Git metadata; only the controller imports exact commits and mutates authoritative refs. Back up remote refs and serialize repository mutations. Successfully readied PR branches remain locked in GitHub until a human explicitly unlocks them. Slack and overlapping pollers are at-least-once delivery and require deduplication. Provider usage collection depends on provider APIs and stale usage must stop expensive scheduling. Prior-case memory can bias an investigation and must be revalidated against current evidence. `gh stack link` is additive, so the approved decomposition and bottom-to-top order must be frozen before submission. Restore can lose changes after the last snapshot. There is no web UI, public ingress, automatic merge, automatic question response, Linear issue creation, or Datadog mutation.
