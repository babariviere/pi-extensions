...
import { createWriteToolDefinition, type ToolDefinition, withFileMutationQueue } from "@earendil-works/pi-coding-agent";

const configuredMaxBytes = Number.parseInt(process.env.CODE_PREVIEW_MAX_WRITE_DIFF_BYTES ?? "", 10);
const MAX_WRITE_DIFF_BYTES =
	Number.isFinite(configuredMaxBytes) && configuredMaxBytes > 0 ? configuredMaxBytes : 200_000;
...
const skipped = (reason: string, byteLength: number | undefined, sizeExceeded = false): ExistingFilePreview => ({
	kind: "skipped",
	reason,
...
export const createPreviewWriteToolDefinition = (
	...
	return {
		...original,
		async execute(_toolCallId: string, params: { path: string; content: string }, signal: AbortSignal | undefined) {
