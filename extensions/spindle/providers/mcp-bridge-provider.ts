...
const descriptors: SpindleActionDescriptor[] = [
  ...
  },
  {
    name: "$list",
    description:
      "List MCP servers and their status as reported by pi-mcp-adapter (does not force a connect)",
    inputSchema: {
      type: "object",
...
export class McpBridgeProvider implements SpindleProvider {
  ...
  readonly description =
    "External MCP tools reached through the pi-mcp-adapter `mcp` gateway tool (lazy connect)";
  ...
  async #gatewayCall(
    ...
    if (details?.isError === true) {
      throw new Error(
        textFromContent(result.content) || "The pi-mcp-adapter gateway returned an error",
      );
    }
