...
import { executionOutcomeFromError, type SpindleExecutionFailureStageV1, type SpindleExecutionTraceOperationHandle } from "./audit/trace.ts";
...
export const HOST_CALLS: readonly HostCall[] = [
  {
    ...
    handle: (args, ctx, signal) =>
      ctx.traceAttempt("spindle.discovery.providers", args, signal, () =>
        ctx.registry
          .providers()
          .filter((provider) => ctx.fullCodeMode || !fullCodeProvider(provider.name)),
      ),
  },
  {
    ref: "spindle.$catalog",
  ...
  },
  {
    ...
    handle: (args, ctx, signal) =>
      ctx.traceAttempt("spindle.discovery.list", args, signal, async (setStage) => {
        ...
        setStage(
          typeof args.provider === "string" && !ctx.registry.has(args.provider)
            ? "resolve"
            : "invoke",
        );
        ...
        return actions.filter(
          (action) => ctx.fullCodeMode || !fullCodeProvider(action.provider),
        );
      }),
  },
  {
...
export const hostCallTable: ReadonlyMap<string, HostCall> = new Map(
  HOST_CALLS.map((call) => [call.ref, call]),
);
