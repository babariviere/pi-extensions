  ...
  defineTool({
    ...
    renderResult(result, { expanded, isPartial }, theme, context) {
      ...
      let audits = restoreLegacyBashCommands(
        details.audits as SpindleRenderAudit[],
        context.args,
      );
      const rendererState = context.state as SpindleRendererState;
      const spinner = updateSpinner(
      ...
      if (isPartial) {
        ...
        if (rendererState.spindleCoreToolPreviews) {
          audits = restoreSpindleCoreToolPreviews(
            audits,
            rendererState.spindleCoreToolPreviews,
          );
        }
        ...
        if (rendererState.spindleCallHeadlinePreviews) {
          audits = restoreSpindleCallHeadlinePreviews(
            audits,
            rendererState.spindleCallHeadlinePreviews,
          );
        }
        ...
        if (audits.length === 0) {
          return trackRows(
            new Text(
              theme.fg(
                "warning",
                `◆ ${safeTerminalText(progress ?? "Running Spindle program…")}`,
              ),
              0,
              0,
            ),
