...
export const createSpindleExecTool = (
	...
	decorateShell(
		defineTool({
			...
			label: "Spindle",
			description:
				"This session's TypeScript code mode and, in full code mode, its exclusive tool interface. Execute type-checked TypeScript in isolated QuickJS to call `pi.*`, `extensions.*`, `mcp.*`, and `agents.*`. Do not use Python as an orchestration fallback.",
