...
import { shouldSkipWriteDiffBytes, shouldSkipWriteDiffComplexity } from "../providers/write-preview.ts";
import { changedLineEmphasis } from "./word-diff/line-emphasis.ts";
import { diffLineNumberWidth, formatDiffLineNumber, parseDiffLine, type ParsedDiffLine } from "./word-diff/parse.ts";
...
type PreviewEntry<T> = { kind: "line"; line: T; index: number } | { kind: "hidden"; hidden: number };
...
const secretWarnings = (source: string): string[] => {
	if (!source) return [];
	const sample =
		source.length <= SECRET_SCAN_CHARS
			? source
...
const renderContent = (
	...
	const skipHighlight = options.settings.syntaxHighlighting && content.length > MAX_HIGHLIGHT_CHARS;
	const language =
		options.settings.syntaxHighlighting && !skipHighlight ? resolveLanguage(filePath, content) : undefined;
	...
	const flush = (): void => {
		...
		const highlighted = language ? highlightCode(normalized.join("\n"), language, options.invalidate) : null;
...
const bashWarnings = (command: string): string[] => {
	...
	const warnings: Array<[string, RegExp]> = [
		["recursive delete", /\brm\b(?=[^;&|]*(?:-[\w-]*r[\w-]*|--recursive)\b)(?=[^;&|]*(?:-[\w-]*f[\w-]*|--force)\b)/i],
...
const injectVisibleRanges = (ansi: string, ranges: Array<[number, number]>, open: string, close: string): string => {
			...
			if (!changed && later) {
				...
				if (later && lines.length > context * 2) {
					...
					out.push(
						...contextLines(
							lines.slice(-context),
							oldLine + lines.length - context,
							newLine + lines.length - context,
						),
					);
				} else {
...
const renderDiff = (
	...
	const limit = Math.max(
		1,
		Math.min(limitOverride ?? toolLimit({ ref: "", tool: "edit" }, options), options.maxLines),
	);
	...
	const skipHighlight = options.settings.syntaxHighlighting && diff.length > MAX_HIGHLIGHT_CHARS;
	const language = options.settings.syntaxHighlighting && !skipHighlight ? resolveLanguage(filePath) : undefined;
		...
		while (start < parsed.length) {
			...
			if (rendered) {
				for (let index = 0; index < content.length; index++) {
					highlighted[start + index] =
						rendered[index] ?? theme.fg("toolOutput", escapeControlChars(content[index] ?? ""));
				}
			}
			start = end;
		}
...
const renderRead = (
	...
	const { content, notice } =
		truncated || typeof audit.args?.limit === "number" ? splitReadContinuationNotice(output) : { content: output };
...
const renderWrite = (
	...
	if (audit.success === true && beforeRecord?.kind === "content") {
		...
		if (beforeContent === undefined) {
			return {
				lines: [theme.fg("success", "✓ Write applied") + theme.fg("muted", " · previous content unavailable")],
				hidden: 0,
			};
		}
		...
		if (shouldSkipWriteDiffBytes(beforeContent, content)) {
			return {
				lines: [theme.fg("success", "✓ Write applied") + theme.fg("muted", " · diff skipped for large content")],
				hidden: 0,
			};
		}
		if (shouldSkipWriteDiffComplexity(beforeContent, content)) {
			return {
				lines: [theme.fg("success", "✓ Write applied") + theme.fg("muted", " · diff skipped for complex rewrite")],
				hidden: 0,
			};
		}
	...
	if (audit.success === true && beforeRecord?.kind === "skipped") {
		...
		if (byteLength !== undefined) {
			reason +=
				beforeRecord.sizeExceeded === true && maxBytes !== undefined
					? ` (${formatBytes(byteLength)} > ${formatBytes(maxBytes)})`
					: ` (${formatBytes(byteLength)})`;
		}
		return {
			lines: [theme.fg("success", "✓ Write applied") + theme.fg("muted", ` · diff skipped: ${reason}`)],
			hidden: 0,
		};
	}
...
const renderGrep = (
	...
	const skipHighlight = options.settings.syntaxHighlighting && output.length > MAX_HIGHLIGHT_CHARS;
...
const pathIcon = (filePath: string, directory: boolean, mode: CodePreviewSettings["pathIcons"]): string => {
	if (mode === "off") return "";
	...
	const flush = (): void => {
		...
		for (const rawPath of chunk) {
			...
			if (!shouldTree) {
				...
				lines.push(`${theme.fg("dim", iconText)}${icon ? " " : ""}${renderPath(body, options.cwd, theme, body)}`);
				continue;
			}
...
const renderBash = (
	...
	const highlightedCommand = command ? highlightCode(command, "bash", options.invalidate) : null;
...
export const coreToolTitle = (
	...
	const durationMs =
		options.settings.toolCallTiming && audit.startedAt !== undefined && audit.endedAt !== undefined
	...
	if (audit.tool === "bash" || audit.tool === "exec") {
		...
		return `${title} ${theme.fg("dim", "$")} ${highlighted ?? theme.fg("accent", escapeControlChars(firstLine))}${metadata(
			theme,
			[
	...
	if (audit.tool === "grep") {
		...
		return `${title} ${theme.fg("accent", `/${escapeControlChars(pattern)}/`)} ${theme.fg("muted", "in")} ${renderPath(filePath || ".", options.cwd, theme)}${metadata(
			theme,
			[glob ? escapeControlChars(glob) : undefined, limit !== undefined ? `limit ${limit}` : undefined, timing],
		)}`;
	}
	if (audit.tool === "find") {
		const pattern = argString(audit, "pattern") || "*";
	...
	if (audit.tool === "write") {
		...
		const byteLength =
			numberOf(rendererPreview?.writeByteLength) ??
			(content !== undefined ? Buffer.byteLength(content, "utf8") : undefined);
		const lineCount =
			numberOf(rendererPreview?.writeLineCount) ?? (content !== undefined ? countContentLines(content) : undefined);
