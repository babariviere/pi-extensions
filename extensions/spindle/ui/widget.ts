...
const totalTokens = (
  ...
  run: SpindleActivityRun | undefined,
): number =>
  snapshot.agents
...
export const shouldShowSpindleWidget = (
  snapshot: SpindleDashboardSnapshot,
  mode: SpindleUiWidgetMode,
): boolean => {
...
export class SpindleWidget implements Component {
  ...
  #leasedRows = 0;
  #pending:
    | { width: number; snapshot: SpindleDashboardSnapshot; lines: string[] }
    | undefined;
