...
export const parseMarkedDiffLine = (rawLine: string): { kind?: DiffLineKind; line: string } => {
...
export const createDiffBackgroundResolver = (
	...
	return (kind) =>
		(cache[kind] ??=
			deriveDiffBg(kind, theme, intensity === "medium" ? 0.24 : 0.14) ?? fallbackDiffBg(kind, intensity));
};

export const applyDiffBackground = (line: string, background: string | undefined): string => {
...
const fallbackDiffBg = (kind: DiffLineKind, intensity: Exclude<DiffBackgroundIntensity, "off">): string => {
	if (kind === "add") {
...
const deriveDiffBg = (kind: DiffLineKind, theme: Theme | undefined, intensity: number): string | undefined => {
...
export const wrapDiffAnsiToWidth = (text: string, width: number, maxRows = 3, continuationPrefix = ""): string[] => {
	if (width <= 0) return [""];
	const rows: string[] = [];
	let row = "";
	let rowWidth = 0;
