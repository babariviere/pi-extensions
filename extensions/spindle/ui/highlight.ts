 ...
 */
export function highlightCode(text: string, lang: string, invalidate?: () => void): string[] | null {
	...
	try {
		...
		const rendered = tokens.map((line) => normalizeContrast(line.map(ansiFromToken).join("")));
