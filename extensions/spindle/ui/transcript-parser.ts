...
import {
	...
	redactRecord,
	terminalSafe,
} from "./transcript-sanitization.ts";
...
export class TranscriptAccumulator {
	...
	#toolParent(id: string): SpindleTranscriptEntry | undefined {
		...
		for (let index = this.#activeTools.length - 1; index >= 0; index--) {
			...
			if (candidate?.toolName === "spindle_exec" && candidate.status === "running") return candidate;
		}
		return undefined;
	}
	...
	#startTool(id: string, label: string, args: unknown): SpindleTranscriptEntry {
		...
		const entry: SpindleTranscriptEntry = {
			...
			...(args !== undefined ? { text: compactRedactedValue(safeArgs ?? redact(args)) } : {}),
	...
	#finishTool(id: string | undefined, label: string, result: unknown, failed: boolean): void {
		...
		if (entry) {
			...
			if (failed && result !== undefined) {
				...
				entry.text = clip(`${entry.text ? `${entry.text} · ` : ""}error: ${failure}`, MAX_TOOL_SUMMARY_CHARS);
			}
		...
		if (event.type === "stream_event") {
			...
			if (stream?.type === "content_block_delta" && delta?.type === "text_delta") {
				...
				if (!this.#assistant) {
					...
					this.#assistant.text = clip(`${this.#assistant.text ?? ""}${text}`, MAX_TRANSCRIPT_MESSAGE_CHARS, false);
				}
			}
			return;
		}
...
export const normalizedToolStarts = (event: Record<string, unknown>): ToolLifecycleStart[] => {
	...
	const appendContentStarts = (content: unknown, type: "toolCall" | "tool_use"): void => {
...
const toolLifecycleEndIds = (event: Record<string, unknown>): string[] => {
	...
	if (event.type === "message") {
		const message = recordOf(event.message);
		return message?.role === "toolResult" && typeof message.toolCallId === "string" ? [message.toolCallId] : [];
	}
