...
export type PreviewLineEntry<T> = { kind: "line"; line: T; index: number } | { kind: "hidden"; hidden: number };
...
function forEachPreviewTextLine(text: string, callback: (line: string, index: number) => void): void {
	let index = 0;
	let pendingEmpty = 0;
