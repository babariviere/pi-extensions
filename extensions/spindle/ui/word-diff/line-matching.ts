...
export function matchChangedLines(
	...
	const removedWeights = removedFeatures.map((tokens) => similarityTokenListWeight(tokens, tokenWeight));
	const addedWeights = addedFeatures.map((tokens) => similarityTokenListWeight(tokens, tokenWeight));
	...
	const similarPairs = prefixAlignedPairs(removed.length, added.length, (removedPosition, addedPosition) => {
...
function competingChangedLineScoreAt(
	...
	for (let candidateRemovedPosition = 0; candidateRemovedPosition < removedLength; candidateRemovedPosition++) {
		if (candidateRemovedPosition === removedPosition || usedRemoved?.has(candidateRemovedPosition)) continue;
...
function addCrossingPairs(
	...
	for (let removedPosition = 0; removedPosition < removed.length; removedPosition++) {
		...
		for (let addedPosition = 0; addedPosition < added.length; addedPosition++) {
			...
			const score = scores[removedPosition]?.[addedPosition] ?? 0;
			if (score >= MIN_CHANGED_LINE_PAIR_SCORE) candidates.push({ removedPosition, addedPosition, score });
		}
	}
	...
	for (const candidate of candidates) {
		if (usedRemoved.has(candidate.removedPosition) || usedAdded.has(candidate.addedPosition)) continue;
		...
		if (candidate.score >= MIN_HIGH_CONFIDENCE_CROSSING_PAIR_SCORE) {
			...
			if (linePairConfidence(candidate.score, availableCompetingScore) === "high") confidence = "high";
		}
...
function addPositionalFallbackPairs(
	...
	for (const [removedPosition, addedPosition] of similarPairs) {
		pairs.push(...positionPairs(removed, added, scores, removedCursor, removedPosition, addedCursor, addedPosition));
		pairs.push([changedLineAt(removed, removedPosition).index, changedLineAt(added, addedPosition).index]);
		removedCursor = removedPosition + 1;
		addedCursor = addedPosition + 1;
	}
	pairs.push(...positionPairs(removed, added, scores, removedCursor, removed.length, addedCursor, added.length));
	return pairs;
}
