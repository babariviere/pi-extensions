...
export function filterLowSignalWordEmphasis(before: string, after: string, ranges: WordChangeRanges): WordChangeRanges {
	...
	return {
		removed: ranges.removed.filter((range) => shouldKeepSmartRange(before.slice(range[0], range[1]), hasAddedSignal)),
		added: ranges.added.filter((range) => shouldKeepSmartRange(after.slice(range[0], range[1]), hasRemovedSignal)),
	};
}
...
function shouldKeepSmartRange(text: string, oppositeSideHasSignal: boolean): boolean {
	...
	const wordTokens = signalTokens.filter((token) => isIdentifierToken(token) || isNumberToken(token));
	const hasIntrinsicSignal = signalTokens.some((token) => isMeaningfulOperatorToken(token) || isSymbolToken(token));
	if (
		...
		return false;
	if (!oppositeSideHasSignal && !hasIntrinsicSignal && isWrapperCallNoise(text, wordTokens)) return false;
	return true;
}
