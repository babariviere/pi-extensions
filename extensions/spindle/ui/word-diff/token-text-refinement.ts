...
import type { TextRange } from "./ranges.ts";
import {
	commonPrefixLength,
	commonSuffixLength,
...
import { isIdentifierToken, isMeaningfulOperatorToken, isNumberToken, type WordEmphasisToken } from "./tokens.ts";
...
function refinedTokenTextRangesByAlignment(
	...
	const pairs = suffixAlignedPairs(beforeSegments.length, afterSegments.length, (before, after) =>
		segmentAt(beforeSegments, before).value === segmentAt(afterSegments, after).value ? 1 : Number.NEGATIVE_INFINITY,
	);
...
function tokenTextGapRanges(
	...
	const removed: TextRange[] =
		beforeStart < beforeEnd ? [[beforeToken.start + beforeStart, beforeToken.start + beforeEnd]] : [];
	const added: TextRange[] =
		afterStart < afterEnd ? [[afterToken.start + afterStart, afterToken.start + afterEnd]] : [];
	return removed.length > 0 || added.length > 0 ? { removed, added } : undefined;
}

function hasPotentialInternalCommonText(before: string, after: string, prefix: number, suffix: number): boolean {
	const beforeMiddle = before.slice(prefix, before.length - suffix);
	const afterMiddle = after.slice(prefix, after.length - suffix);
	const [shorter, longer] =
		beforeMiddle.length <= afterMiddle.length ? [beforeMiddle, afterMiddle] : [afterMiddle, beforeMiddle];
...
function textSegmentOffset(segments: TextBoundarySegment[], index: number, textLength: number): number {
...
function shouldRefineTokenText(before: string, after: string, prefix: number, suffix: number): boolean {
	...
	if (isIdentifierToken(before) && isIdentifierToken(after)) {
		if (sharedEdgeLength < 2 && !needsBoundarySafeOffsets(before) && !needsBoundarySafeOffsets(after)) return false;
