...
import { suffixAlignmentScore } from "./alignment.ts";
import { wordEmphasisSimilarityTokenValues, wordEmphasisTokenWeight } from "./tokens.ts";
import { changedLineTokens, type IndexedChangedLine } from "./changed-line.ts";
...
function changedLineSimilarityTokenValues(line: IndexedChangedLine<AddedDiffLine | RemovedDiffLine>): string[] {
	return (line.similarityTokenValues ??= wordEmphasisSimilarityTokenValues(changedLineTokens(line)));
}

function changedLineSimilarityFeatureValues(line: IndexedChangedLine<AddedDiffLine | RemovedDiffLine>): string[] {
	return (line.similarityFeatureValues ??= similarityFeatures(changedLineSimilarityTokenValues(line)));
}
...
function countSimilarityDocuments(featureLists: string[][], documentCounts: Map<string, number>): void {
	for (const features of featureLists) {
		for (const feature of new Set(features)) documentCounts.set(feature, (documentCounts.get(feature) ?? 0) + 1);
	}
}
...
function similarityFeatures(tokens: string[]): string[] {
	...
	appendSimilarityShingles(features, tokens.filter(isSimilarityShingleToken), 2, SIMILARITY_BIGRAM_PREFIX);
	return features;
}

function appendSimilarityShingles(features: string[], tokens: string[], size: number, prefix: string): void {
...
export function similarityTokenWeight(documents: ChangedLineSimilarityDocuments): SimilarityTokenWeight {
...
export function tokenSimilarity(
	...
	const bagSimilarity = unorderedTokenSimilarity(beforeTokens, afterTokens, weight, beforeWeight, afterWeight);
	...
	return Math.max(orderedSimilarity, bagSimilarity * 0.8, orderedSimilarity * 0.75 + bagSimilarity * 0.25);
}
...
function orderedTokenSimilarity(
	...
	const score = suffixAlignmentScore(beforeTokens.length, afterTokens.length, (beforeIndex, afterIndex) => {
