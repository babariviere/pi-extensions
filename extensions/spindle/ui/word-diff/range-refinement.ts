...
import { commonPrefixLength, commonSuffixLength, rangesAtGraphemeBoundaries } from "./text-boundaries.ts";
import { collectChangedTokenGaps, type ChangedTokenGap } from "./token-alignment.ts";
import {
...
import type { WordChangeRanges } from "./types.ts";
import { suffixAlignedPairs } from "./alignment.ts";
import { refinedTokenTextRanges } from "./token-text-refinement.ts";
...
function refinedSingleTokenRanges(
	...
	if (beforeGroup.end - beforeGroup.start !== 1 || afterGroup.end - afterGroup.start !== 1) return undefined;
	return refinedTokenPairRanges(tokenAt(beforeTokens, beforeGroup.start), tokenAt(afterTokens, afterGroup.start));
}
...
function refinedTokenPairRanges(
	...
	if (identifierRanges && isNarrowerThanWholeTokens(identifierRanges, beforeToken, afterToken)) {
		if (shouldSuppressUnbalancedIdentifierPartRefinement(beforeToken, afterToken, textRanges)) return textRanges;
		if (
			textRanges &&
...
function softAlignedTokenPairs(before: WordEmphasisToken[], after: WordEmphasisToken[]): Array<[number, number]> {
	return suffixAlignedPairs(before.length, after.length, (beforeIndex, afterIndex) => {
		const substitution = softTokenSubstitutionWeight(tokenAt(before, beforeIndex), tokenAt(after, afterIndex));
		return substitution > 0 ? substitution : Number.NEGATIVE_INFINITY;
	});
}

function softTokenSubstitutionWeight(beforeToken: WordEmphasisToken, afterToken: WordEmphasisToken): number {
	...
	return similarity >= MIN_SOFT_TOKEN_SUBSTITUTION_SIMILARITY
		? Math.min(wordEmphasisTokenWeight(beforeToken.value), wordEmphasisTokenWeight(afterToken.value)) * similarity
		: 0;
}

function softTokenSimilarity(before: string, after: string): number {
	if (isIdentifierToken(before) && isIdentifierToken(after)) return identifierTokenSimilarity(before, after);
	if (isNumberToken(before) && isNumberToken(after)) return edgeTextSimilarity(before, after);
	if (isMeaningfulOperatorToken(before) && isMeaningfulOperatorToken(after)) return edgeTextSimilarity(before, after);
	return 0;
}
...
function refinedIdentifierTokenRanges(
	...
	if (!isIdentifierToken(beforeToken.value) || !isIdentifierToken(afterToken.value)) return undefined;
	...
	collectChangedTokenGaps(beforeParts, 0, beforeParts.length, afterParts, 0, afterParts.length, gaps);
