...
const configuredDiffWrapRows = Number.parseInt(process.env.CODE_PREVIEW_DIFF_WRAP_ROWS ?? "", 10);
const DIFF_WRAP_ROWS =
	Number.isFinite(configuredDiffWrapRows) && configuredDiffWrapRows > 0 ? configuredDiffWrapRows : 3;
...
const INHERITED_TEXT_RESET_PARAMETERS = ["22", "23", "24", "25", "27", "28", "29", "39", "54", "55"];
...
class BoundedLineList implements Component {
		...
		for (let lineIndex = 0; lineIndex < this.lines.length; lineIndex++) {
			...
			if (!kind) {
				...
				if (this.wrapLineIndexes?.has(lineIndex)) {
					...
					const wrapped = wrapTextWithAnsi(line, Math.max(1, width - visibleWidth(continuationIndent)));
					renderedRows = wrapped.map((row, index) => (index === 0 ? row : continuationIndent + row));
			...
			const continuationPrefix = pipe < 0 ? "" : " ".repeat(visibleWidth(line.slice(0, pipe + 2)));
	...
	withInheritedBackground(): BoundedLineList {
		...
		return new BoundedLineList(this.lines, this.theme, this.diffIntensity, this.wrapLineIndexes, true);
	}
...
const visibleMulticallAudits = (
	...
	const selected = new Set(active.slice(-limit).map(({ auditIndex }) => auditIndex));
...
const digestCommand = (command: string): string => `sha256:${createHash("sha256").update(command).digest("hex")}`;
...
const renderWriteArgumentBody = (
	...
	return {
		lines: shown.map((line, index) => highlighted?.[index] ?? theme.fg("toolOutput", line || " ")),
...
export const renderSpindleWriteArgumentPreview = (
	...
	for (let index = 0; index < shownBindings.length; index++) {
		...
		if (index === activeIndex) {
			...
			if (body.hidden > 0) {
				rows.push(theme.fg("dim", `  … ${body.hidden} more ${body.hidden === 1 ? "line" : "lines"}`));
			}
		}
	}
...
const shortIdOf = (value: unknown): string | undefined => (typeof value === "string" ? value.slice(0, 8) : undefined);

const countOf = (result: unknown): string => (Array.isArray(result) ? String(result.length) : "");
...
const callHeadlinePreview = (audit: SpindleRenderAudit): string | undefined => {
	...
	return (
		providerCallDetail(provider, tool, args, audit.result, audit.preview) ||
		headlineArg(args) ||
...
export function nestedCallTitle(
	...
	const args = audit.args ?? {};
	const providerDetail = providerCallDetail(provider, tool, args, audit.result, audit.preview, audit.previewHeadline);
	...
	if (path) detail = path;
		...
		detail =
			headlineArg(args) || structuralCallDetail(provider, tool, args, audit.result) || audit.previewHeadline || "";
	}
...
export const renderNestedAgentToolLines = (
	...
	for (let toolIndex = 0; toolIndex < visibleTools.length; toolIndex++) {
		...
		const title = nestedCallTitle(nestedAudit, theme, options.invalidate, options.core);
		if (lines.length === 0) {
		...
		if (!options.expanded || !options.core || toolIndex < visibleTools.length - EXPANDED_AGENT_TOOL_BODY_COUNT)
			continue;
...
export const singleCallProgressLine = (progress: string | undefined, nestedLines: string[]): string =>
	progress && nestedLines.length === 0 ? safeTerminalText(progress) : "";
...
export const renderSpindleMulticallPartial = (
	...
	for (let visibleIndex = 0; visibleIndex < callsShown.length; visibleIndex++) {
		...
		if (audit.success !== false && input.preview?.auditIndex === auditIndex) {
			...
			if (input.preview.hidden > 0) {
				rows.push(
					theme.fg("dim", `  … ${input.preview.hidden} more ${input.preview.hidden === 1 ? "line" : "lines"}`),
				);
			}
		}
	...
	if (callsHidden > 0) {
		...
		rows.push(theme.fg("dim", label) + (input.expanded ? "" : theme.fg("dim", " · ") + expandHint(theme)));
	}
...
export const captureSpindleCoreToolPreviews = (
	...
	return audits.flatMap((audit) => {
		...
		const index = prior.findIndex(
			(candidate) => candidate.ref === audit.ref && argString(candidate.args ?? {}, "path") === path,
		);
...
export const restoreSpindleCoreToolPreviews = (
	...
	return audits.map((audit) => {
		...
		let index = remaining.findIndex(
			(preview) => preview.ref === audit.ref && argString(preview.args ?? {}, "path") === path,
		);
...
export const restoreSpindleWritePreviews = (
	...
	return audits.map((audit) => {
		...
		const path = argString(audit.args ?? {}, "path");
		const index = remaining.findIndex((preview) => preview.ref === audit.ref && preview.path === path);
		if (index < 0) return audit;
		const [preview] = remaining.splice(index, 1);
...
const lineDiff = (oldLines: string[], newLines: string[]): DiffLine[] => {
	...
	for (let i = m - 1; i >= 0; i--) {
		...
		for (let j = n - 1; j >= 0; j--) {
			...
			cur[j] = oldLine === newLine ? (next[j + 1] ?? 0) + 1 : Math.max(next[j] ?? 0, cur[j + 1] ?? 0);
		}
	}
	const out: DiffLine[] = [];
	let i = 0;
...
// Mirrors pi core's read range notice: surface how many lines a spindle_exec
