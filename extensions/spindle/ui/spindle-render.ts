...
export const renderSpindleWriteArgumentPreview = (
  ...
  const rows = [
    theme.fg(
      "warning",
      `◆ Spindle composing · ${completed}/${input.bindings.length} writes`,
    ),
  ];
...
export const renderSpindleMulticallPartial = (
  ...
  let header = theme.fg(
    "warning",
    `◆ Spindle running · ${done}/${input.audits.length} calls`,
  );
...
export const captureSpindleCallHeadlinePreviews = (
  audits: SpindleRenderAudit[],
): SpindleCallHeadlinePreview[] =>
  audits.flatMap((audit) => {
