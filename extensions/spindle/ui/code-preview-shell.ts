...
const updateTiming = (
	...
	if (context.executionStarted && state.codePreviewTimingStartedAt === undefined && context.isPartial !== false) {
...
const renderTimedResult = (
	...
	const component =
		...
		render({
			...context,
			lastComponent: unwrapTimingComponent(state.codePreviewTimingResultComponent ?? context.lastComponent),
		});
...
const borderState = (context: PreviewRenderContext): BorderState => context.state as BorderState;
...
class BorderedToolCall implements Component {
	...
	private renderUncached(width: number): string[] {
		...
		const top =
			labelWidth > 0 && labelWidth <= width - 2
...
export const withCodePreviewShell: SpindleToolShellDecorator = (tool, options = {}) => {
	...
	return {
		...
		renderCall(args, theme, rawContext) {
			...
			if (mode !== "border") {
				const state = timingState(context);
				if (context.isPartial && isTimingOnlyRender(state) && state?.codePreviewTimingCallComponent) {
				...
				const wrapped = state ? new TimingPreservedComponent(component, state) : component;
			...
			const component =
				timingOnly && state.codePreviewBorderCallComponent
			...
			const shell =
				state.codePreviewBorderShell instanceof BorderedToolCall && state.codePreviewBorderTheme === theme
		...
		renderResult(result, resultOptions, theme, rawContext) {
			...
			const label = updateTiming(context, timingEnabled);
			if (mode !== "border") {
				return renderTimedResult(context, theme, (next) => renderResult(result, resultOptions, theme, next), label);
			}

			const state = borderState(context);
			const timingOnly = context.isPartial && isTimingOnlyRender(state);
			const component =
				timingOnly && state.codePreviewBorderResultComponent
