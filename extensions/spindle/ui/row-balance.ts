...
export const resultRowDeficit = (balance: ResultRowBalance, width: number): number => {
	if (!balance.finalized || !balance.partial || !balance.final) return 0;
	const partialRows =
		balance.partial.width === width ? balance.partial.rows : balance.partial.component.render(width).length;
