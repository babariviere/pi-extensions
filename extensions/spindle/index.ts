...
import {
  SpindleToolLifecycle,
  SpindleToolOwnership,
  ownsSpindleToolSource,
} from "./core/tool-ownership.ts";
import {
  expandSkillDirMarkersForRead,
  expandSkillDirMarkersInSkillBlock,
} from "./core/skill-dir.ts";
...
import {
  SPINDLE_PROVIDER_REGISTER_EVENT,
  type SpindleProviderRegistration,
} from "./protocol.ts";
...
export default async function spindle(pi: ExtensionAPI): Promise<void> {
  ...
  pi.on("session_start", async (_event, context) => {
    ...
    try {
      ...
      Object.assign(
        spindleTool,
        createSpindleExecTool(state, codePreviewSettings, decorateShell),
      );
    } catch (error) {
  ...
  pi.on("before_agent_start", async (event) => {
    const fullCodeMode = state.initialized
      ...
      : DEFAULT_SPINDLE_CONFIG.fullCodeMode;
    ...
    const skillReferenceGuidance = fullCodeMode
