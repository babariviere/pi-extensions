...
const FULL_CODE_GUIDANCE =
	...
	"Read tools return text. `pi.bash`, `pi.edit`, and `pi.write` return `{ok, output, details}`. Use `payloads` and `π.key` for multiline or long values. Use `τ` for a large intermediate needed in a later call. `tools.search()` discovers extension and MCP actions, not repository contents. Use `mcp.*` for MCP and `agents.*` for subagents. See the `spindle-exec` skill only when detailed API guidance is needed.";
