...
const FULL_CODE_GUIDANCE =
	...
	"Read tools return text. `pi.bash`, `pi.edit`, and `pi.write` return `{ok, output, details}`. Use `payloads` and `π.key` for multiline values. For external capabilities, discover before declaring them unavailable: `tools.search({query})` finds registered actions, while `mcp.list()` or `mcp.search({query})` finds lazy MCP services such as Slack. Connect with `mcp.connect('slack')` if needed, then search and describe its action before `mcp.call`. Use `agents.*` for subagents.";
