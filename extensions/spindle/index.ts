...
const FULL_CODE_GUIDANCE =
	...
	"Return shapes follow one rule, by category: reads hand back their data as a bare string (`pi.read`, `pi.grep`, `pi.find`, `pi.ls`); mutations hand back an outcome envelope `{ok, output, details}` (`pi.bash`, `pi.edit`, `pi.write` — read `.output`); out-of-process calls hand back a run handle (`agents.*` — `{runId, state}`, where `state: 'running'` means the wait window expired, not that anything failed); foreign providers pass their own envelope through unchanged (`extensions.*` gives `{content, text, isError}`, `mcp.*` gives `{text, content, structuredContent}`).\n" +
	"Examples: `pi.read('/x')`, `pi.grep({regex:'TODO', ic:true, ctx:2})`, `pi.find('*.ts')`, `pi.ls('src')`, `pi.bash({cmd:'ls'})`, `pi.edit({path:'/x', old:'a', new:'b'})`, `pi.write({path:'/y', text:'z'})`. Failed core calls reject, including `bash` on an ordinary nonzero exit; pass `settle: true` to `pi.bash` to get `{ok: false, exitCode, output, error}` instead. `pi.bash` also accepts per-call `cwd` (absolute), `env` (merged over the shell environment), and `stdin` (piped text: `pi.bash({cmd: 'ssh host bash -s', stdin: π.script})`); `process.env` holds an allowlisted host env snapshot (HOME, USER, SHELL, PWD, PATH, LANG, LC_*, TERM, TMPDIR, XDG_*) and `process.cwd()` the session directory. Timeout, cancellation, approval, and security failures still reject.\n" +
	"Other namespaces: `extensions.<tool>(args)` for tools registered by sibling extensions; `tools.list()` / `tools.search({query})` / `tools.describe({ref})` / `tools.call({ref, args})` for the static action registry, where `tools.call` is for a ref computed at runtime; `mcp.call(server, tool, args)` is the only way to call an MCP tool, with `mcp.list()` / `mcp.search()` / `mcp.describe()` / `mcp.connect()` for MCP's own lazy catalog (servers connect on demand, so MCP tools are not in the static registry); `agents.list()` / `agents.run({agent, task})` / `agents.runAll({tasks})` for custom markdown subagents (a result with `state: 'running'` is not a failure: the wait window expired, so resume with `agents.wait({runId})` or stop it with `agents.cancel({runId})`). `print(...)` logs; `π.<key>` reads the `payloads` parameter (it is not a tool).\n" +
	"`τ` is the session scratchpad shared across `spindle_exec` calls: `await τ.set('key', value)` / `await τ.get('key')` / `await τ.keys()` / `await τ.delete('key')`, JSON values only. Use it to hand a large intermediate to a later program instead of re-deriving it or routing it through context; return small values outright and use a file for anything big or long-lived. Writes can fail (non-serializable value, budget) and the held keys are echoed as `τ keys:` in every result.";
