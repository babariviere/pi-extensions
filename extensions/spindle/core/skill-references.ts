...
const NEGATED_INVOCATION = /^\s*(?:[-*]\s*)?(?:do not|don't|never)\s+(?:run|invoke|load|start|follow|use)\b/i;
...
export const buildSkillReferenceGuidance = (prompt: string, skills: readonly SkillReference[]): string | undefined => {
	...
	return [
		...
		mappings,
		"Load each mapped SKILL.md through pi.read inside spindle_exec and follow it. Skill loading is a dependency, not task exploration. Continue the active workflow after tool results and later user replies until it completes.",
	].join("\n");
};
