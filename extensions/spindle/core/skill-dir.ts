...
export const expandSkillDirMarkers = (content: string, skillDir: string): string =>
	content.replaceAll(SKILL_DIR_MARKER, skillDir);
...
export const expandSkillDirMarkersInSkillBlock = (content: string): string => {
	...
	return expandSkillDirMarkers(content.slice(0, skillEnd), path.dirname(block.location)) + content.slice(skillEnd);
};
...
export const expandSkillDirMarkersForRead = (content: string, args: Record<string, unknown>, cwd: string): string => {
