...
const listHas = (list: readonly string[] | undefined, tool: string): boolean =>
	(list ?? []).some((entry) => lower(entry) === lower(tool));
...
const inferServer = (tool: string, config: McpReadOnlyConfig): string | undefined => {
	const names = new Set([...Object.keys(BUILTIN_MCP_SERVER_POLICIES), ...Object.keys(config.servers)]);
	for (const name of names) {
		const policy = policyFor(name, config);
		if (listHas(policy.deny, tool) || listHas(policy.allow, tool)) return name;
	}
	return undefined;
};
...
export const decideMcpCall = (input: {
	...
	const shape = classifyMcpToolName(tool);
	if (shape === "write") {
 ...
 */
export const assertMcpGatewayArguments = (gate: McpReadOnlyGate, args: Record<string, unknown>): void => {
	const tool = typeof args.tool === "string" && args.tool.trim() ? args.tool.trim() : undefined;
	if (!tool) return;
	gate.assert(tool, typeof args.server === "string" ? args.server : undefined);
};
