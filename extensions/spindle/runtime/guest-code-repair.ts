...
const isIdentChar = (char: string | undefined): boolean => char !== undefined && /[A-Za-z0-9_$]/.test(char);
...
const skipBalanced = (code: string, index: number, end: number, open: string, close: string): number => {
	let depth = 1;
	let i = index;
	while (i < end && depth > 0) {
		const char = code[i];
...
const collectTemplateRepairs = (code: string, index: number, end: number, repairs: Repair[]): number => {
	let i = index + 1;
	while (i < end) {
		const char = code[i];
		if (char === "\\") {
