...
export const ORCHESTRATOR_CONTRACT = [
	...
	"- One approved ledger item is one subagent run. Delegate it with the `run` tool of the `agents` provider.",
	"- Always pass both `night: true` and `nightTodoId: 'TODO-<id>'`. The provider rejects a launch whose id was not",
	...
	const limit =
		input.reason === "pacing"
			...
			: input.reason === "week"
				...
				: "5h usage window";
	const lines = [
		`[night-mode] ${input.reason === "pacing" ? "Manual" : "Automated"} resume: the ${input.reason === "pacing" ? "Codex" : "Claude"} ${limit} has room again. This message was generated by the ` +
