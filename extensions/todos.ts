/**
 * Origin: https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/todos.ts
 * Vendored from Armin Ronacher's `mitsupi` package (MIT).
 *
...
type KeybindingMatcher = {
	// Accept `any` for the keybinding id so this structural type stays compatible
	// with the SDK's KeybindingsManager, whose param is typed as `keyof Keybindings`.
...
export default function todosExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("todos", {
		...
		handler: async (args, ctx) => {
			...
			// `null as TUI | null` avoids a control-flow quirk where TS narrows a
			// closure-assigned `let` to `never` at the later optional-chained read.
