...
export function matchChangedLinesSparse(
	...
	const scoreAt = (removedPosition: number, addedPosition: number): number => {
		...
		const addedWeight = (addedWeights[addedPosition] ??= similarityTokenListWeight(addedFeatures, tokenWeight));
	...
	for (let index = 0; index < Math.min(removed.length, added.length); index++) {
		...
		const competingScore = policy.competingChangedLineScoreAt(removed.length, added.length, index, index, scoreAt);
...
function sparseChangedLinePairCandidates(
	...
	for (const [feature, featureRemovedPositions] of removedPositions) {
		...
		const uniqueFeature =
			documentCount === 2 && featureRemovedPositions.length === 1 && featureAddedPositions.length === 1;
...
export function competingCandidateValue(values: TopTwoCandidateValues | undefined, candidateValue: number): number {
	if (!values) return 0;
...
function boundedSparseChangedLinePairCandidates(
	...
	return candidates.filter((candidate) => selectedByRemoved.has(candidate) && selectedByAdded.has(candidate));
}
...
function topSparseCandidates(
	...
	for (const atPosition of candidates.values()) {
		...
		for (const candidate of atPosition.slice(0, MAX_SPARSE_CANDIDATES_PER_LINE)) selected.add(candidate);
	}
	return selected;
}

function compareSparseCandidates(a: SparseChangedLinePairCandidate, b: SparseChangedLinePairCandidate): number {
	return (
...
function sparsePositionalCompetingScore(
	...
	if (candidate.addedPosition < removedLength && candidate.removedPosition !== candidate.addedPosition)
		competingScore = Math.max(competingScore, scoreAt(candidate.addedPosition, candidate.addedPosition));
	return competingScore;
}

function hasStrongSparseEvidence(candidate: SparseChangedLinePairCandidate): boolean {
	if (!candidate.hasUniqueFeature && candidate.sharedFeatureCount < MIN_SPARSE_RARE_FEATURE_COUNT) return false;
	return (
