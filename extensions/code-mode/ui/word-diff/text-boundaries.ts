...
export function rangesAtGraphemeBoundaries(text: string, ranges: Array<[number, number]>): Array<[number, number]> {
...
function commonSuffixCodeUnitLength(before: string, after: string, prefixLength: number): number {
	...
	let length = 0;
	while (length < maxLength && before[before.length - 1 - length] === after[after.length - 1 - length]) length++;
	return length;
}
...
function graphemeBoundaryAtOrAfter(segments: GraphemeSegments, offset: number, textLength: number): number {
	if (offset <= 0) return 0;
...
function graphemeStartAtOrBefore(segments: GraphemeSegments, offset: number, textLength: number): number {
	if (offset <= 0) return 0;
	if (offset >= textLength) return textLength;
	return segments.containing(offset)?.index ?? offset;
}

function graphemeEndAtOrAfter(segments: GraphemeSegments, offset: number, textLength: number): number {
	if (offset <= 0) return 0;
	if (offset >= textLength) return textLength;
	const segment = segments.containing(offset - 1);
