...
import { wordEmphasisTokenWeight, type WordEmphasisToken } from "./tokens.ts";
import { suffixAlignedPairs } from "./alignment.ts";
import type { TokenGroup } from "./ranges.ts";
...
export function collectChangedTokenGaps(
	...
	confidence = lowerWordChangeConfidence(
		confidence,
		collectChangedTokenGaps(before, previousBefore, beforeEnd, after, previousAfter, afterEnd, gaps),
	);
	return lowerWordChangeConfidence(confidence, "medium");
}

function lowerWordChangeConfidence(a: WordChangeConfidence, b: WordChangeConfidence): WordChangeConfidence {
...
function collectChangedTokenGapsByLcs(
	...
	appendChangedTokenGap(gaps, beforeStart + beforeIndex, beforeEnd, afterStart + afterIndex, afterEnd);
}
...
function uniqueOrderedAnchors(
	...
	for (let index = afterStart; index < afterEnd; index++) {
		...
		if (beforeCounts.get(value) === 1 && afterCounts.get(value) === 1) afterUniqueIndexes.set(value, index);
	}
