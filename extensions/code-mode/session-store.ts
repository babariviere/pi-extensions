 ...
 *
 * A `code_mode` program runs in a fresh QuickJS context, so everything it
 ...
 * model cannot see state it did not write in this turn, so `code_mode` echoes
 * the held keys in every result (`execution-service.ts` reads `keys()`,
 * `code-mode-tool.ts` renders them).
 */
