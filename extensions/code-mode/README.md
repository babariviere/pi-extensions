...
In TUI mode, the widget above the editor shows only running jobs and subagents. Jobs show their name, short id, and elapsed time, and disappear when they exit or are stopped. Ordinary `code_mode` programs and nested calls stay in the tool call block; `display.name` is its title and `display.description` appears as a one-line objective above the TypeScript preview. The widget obeys the existing `ui.enabled`, `ui.widget`, and `ui.maxRows` settings; `jobs.status()` remains available for a complete history.
...
For routine delegation, omit `model` and `thinking` from `agents.run`, `agents.runAll`, and `agents.start`. A named agent's model and thinking settings take precedence over `agents.defaultModel` and `agents.defaultThinking`; missing settings fall back to those defaults. The generic `task` agent uses the configured model or the parent model. Reserve per-run overrides for unusually demanding tasks that need a specific model or reasoning level.
