...
The `code_mode` named-payload argument is `payloads`. Compatibility
...
Full-code prompt guidance identifies `code_mode` as the session's TypeScript
...
UTF-8 bytes and points to the available `code-mode` skill through `pi.read`,
...
[`skills/code-mode`](../../skills/code-mode/SKILL.md), whose references
...
Final `code_mode` details also carry optional versioned `editMetrics`. New
...
- `mapLimit(items, fn, N)` — bounded-concurrency fan-out, the one concurrency
  ...
  and the one taught spelling per tool lives in `skills/code-mode`, loaded
...
Upstream path → local path is `src/X` → `extensions/code-mode/X` for everything in
...
```sh
fd -e ts . extensions/code-mode -x perl -pi -e 's{(from\s+")(\.\.?/[^"]*)\.js(")}{$1$2.ts$3}g'
```
...
original core tools with no options, so a `pi.bash` inside `code_mode` had the
...
| `write`, `edit`, `applyPatch` | direct path check against the write allowlist | They never reach a shell, so the check is exact and needs no OS support. `applyPatch` preflights all paths and rechecks each operation's guard before IO; file operations run sequentially and may leave partial changes on failure |
...
inside `code_mode`, so the agent cannot request its own sandbox. Payloads are
...
`extensions/code-mode/agents/` holds the **absorbed subagents extension**. It is
...
implementation now lives in `extensions/code-mode/agents/` and is reachable only
...
A subagent run used to be tied to the `code_mode` program that started it,
...
6. Verify: `npm run typecheck`, `npm test` and `npm run fmt:check` must all exit
   ...
   ```sh
   fd fabric extensions/code-mode skills          # must be empty
   rg -i fabric extensions/code-mode skills -g '!CONTEXT.md' -g '!guest-host-refs.test.ts'   # must be empty
   rg 'fabric\.\$' extensions/code-mode -g '!guest-host-refs.test.ts'   # must be empty
   ```
