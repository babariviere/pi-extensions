...
import { createCodeModeBashToolDefinition, MAX_STDIN_CHARS } from "./code-mode-bash-tool.ts";
...
test("extras-free calls run unchanged on the base tool", async () => {
	...
	const tool = createCodeModeBashToolDefinition(process.cwd(), {
...
test("cwd and env extras are applied and merged over the shell env", async () => {
	...
	await tool.execute(
		"t2",
		{ command: "pwd", cwd: "/tmp", env: { PI_CODE_MODE_TEST_VAR: "1" } },
		undefined,
		undefined,
		undefined as never,
	);
	...
	assert.equal(seen[0].env?.PI_CODE_MODE_TEST_VAR, "1");
...
test("stdin, cwd, and env combine", async () => {
	...
	const result = await tool.execute(
		"t4",
		{ command: "cat; pwd", cwd: "/tmp", env: { PI_CODE_MODE_TEST_VAR: "1" }, stdin: "x\n" },
		undefined,
		undefined,
		undefined as never,
	);
