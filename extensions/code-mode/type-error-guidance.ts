...
const CODE_MODE_EXEC_ARGUMENT_NOTES: Readonly<Record<string, string>> = {
	...
	timeoutMs: "`timeoutMs` belongs to the outer `code_mode` call, not inside `code`.",
	display: "the `display` objective belongs to the outer `code_mode` call, not inside `code`.",
	resultFormat: "`resultFormat` belongs to the outer `code_mode` call, not inside `code`.",
};
