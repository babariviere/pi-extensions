...
const raceWithAbort = <T>(operation: PromiseLike<T>, signal: AbortSignal | undefined): Promise<T> => {
...
export const runAbortable = <T>(signal: AbortSignal | undefined, operation: () => T | PromiseLike<T>): Promise<T> => {
	try {
...
export const settleWithin = async (operations: Iterable<PromiseLike<unknown>>, timeoutMs: number): Promise<boolean> => {
