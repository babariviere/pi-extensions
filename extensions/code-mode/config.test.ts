...
import { normalizeCodeModeConfig } from "./config.ts";

test("stale sandbox network keys stay inert through normalizeCodeModeConfig", () => {
	// A code-mode.json written before the Seatbelt migration may still carry
	// these keys. There is no schema validation on this path
	// (readJsonObjectFile / normalizeCodeModeConfig), so an unknown key must
	// simply be ignored rather than throwing or resurrecting network config.
	const config = normalizeCodeModeConfig({
