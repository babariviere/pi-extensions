...
test("a caller-supplied map is honored for caller-supplied code", async () => {
	const runtime = new QuickJsRuntime();
	const { transpileCodeModeCode } = await import("./type-checker.ts");
...
for (const code of [
	"return await mapLimit([1, 2, 3, 4, 5], async (n) => n * 2, 2);",
	"return await Promise.all([1, 2, 3, 4, 5].map(async (n) => n * 2));",
]) {
	test(`fan-out runs without display host calls: ${code}`, async () => {
		const calls: string[] = [];
...
test("mapLimit preserves mapper rejections without display calls", async () => {
	const result = await new QuickJsRuntime().execute(
		"await mapLimit([1, 2, 3, 4], (n) => { if (n === 3) throw new Error('boom'); return n; }, 2);",
		unexpectedHostCall,
		baseOptions,
	);
	assert.equal(result.terminationReason, "runtime_error");
